...
async function runAutomatedTask() {
    ...
    try {
        ...
        for (const page of situations) {
            ...
            const title = props.Title_KR?.title?.[0]?.plain_text || props.Name?.title?.[0]?.plain_text || '';
        ...
        console.log('After generating each image, save it to the "public/situations/" folder with the corresponding ID name.');
        console.log('Then call "node scripts/upload-images.js <FILENAME>" to upload it to Vercel Blob.');
