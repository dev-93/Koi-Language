...
import 'swiper/css';
import { ChevronLeft, ChevronDown, ChevronUp, Sparkles, Volume2, Heart } from 'lucide-react';
...
export default function LearnView({ situation, initialExpressions = [] }) {
    ...
    useEffect(() => {
        ...
        setExpressions(initialExpressions?.length > 0 ? initialExpressions : []);
    ...
    return (
        <div
            className="home-layout relative bg-white"
            style={{ paddingBottom: '80px', overflow: 'auto' }}
        >
            <header
                className="w-full max-w-[480px] d-flex items-center justify-between px-4 bg-white/90 backdrop-blur sticky top-0 z-40 border-b border-gray-50"
                style={{ padding: '8px 16px' }}
            >
                <button
                    onClick={() => router.push('/')}
                    className="p-2 u-rounded-full hover:bg-gray-50 border-none bg-transparent cursor-pointer transition-all active:scale-95"
                >
                ...
                <button
                    ...
                    className="px-3 py-1 u-shadow-sm border-2 border-peach/20 u-rounded-full font-black text-[11px] text-peach hover:bg-peach/5 transition-all cursor-pointer bg-white active:scale-95"
                >
                    {isKr ? 'KR' : 'JP'}
                </button>
            </header>
                ...
                <div className="w-full max-w-[440px]">
                    ...
                    >
                        {expressions.map((expr, idx) => {
                            ...
                            const readingText =
                                parseValue(
                                    expr.reading || expr.pron || expr.Reading || expr.pronunciation,
                                    isKr ? 'kr' : 'jp'
                                ) || expr.reading_en;
                            ...
                            return (
                                <SwiperSlide key={expr.id || idx}>
                                    <div
                                        className={`m-1 px-3 py-3 u-rounded-[28px] shadow-2xl transition-all duration-500 border border-gray-50 bg-white d-flex flex-col items-center ${currentIndex === idx ? 'scale-100 opacity-100' : 'scale-[0.92] opacity-40 blur-[1px]'}`}
                                    >
                                        <SituationScene
                                        ...
                                        <div className="w-full d-flex flex-col items-center justify-center mt-2">
                                            <div className="d-flex items-center gap-2">
                                                <h2 className="learn-expr-main">{mainText}</h2>
                                                <div
                                                    className="d-flex flex-col items-center shrink-0"
                                                    style={{ gap: '6px' }}
                                                >
                                                    <button
                                                        onClick={() =>
                                            ...
                                            {readingText && (
                                                <p className="learn-expr-reading">{readingText}</p>
                                            )}

                                            <p className="learn-expr-sub">{subText}</p>
                                        </div>
                                        ...
                                        <div className="w-full px-2 d-grid grid-cols-3 gap-2">
                                            {wordList.map((word, wIdx) => (
                                                <div
                                                    key={wIdx}
                                                    className="d-flex flex-col items-center u-rounded-2xl overflow-hidden border border-gray-100 shadow-sm bg-white p-2"
                                                >
                                                    <span className="text-[13px] font-black text-gray-800 text-center w-full truncate">
                                                        {word.main}
                                                    </span>
                                                    {word.extra && (
                                                        <span className="text-[9px] text-gray-300 font-bold mt-1 italic text-center w-full truncate">
                                                            {word.extra}
                                                        </span>
                                                    )}
                                                    <span className="text-[11px] font-bold text-peach text-center w-full truncate mt-0.5">
                                                        {word.sub}
                                                    </span>
                                                </div>
                                            ))}
